create or replace function app_private.get_provider_quote_opportunity_authoritatively(p_request_id uuid, p_provider_id uuid)
...
drop function public.get_provider_quote_opportunity(uuid,uuid);
create function public.get_provider_quote_opportunity(p_request_id uuid, p_provider_id uuid)
returns table(request_id uuid, need_text text, request_state public.request_state, service_entity_id uuid, location_id uuid, market_id uuid)
language sql security invoker set search_path='app_private' as $$
  select * from app_private.get_provider_quote_opportunity_authoritatively(p_request_id,p_provider_id)
$$;
