create or replace function public.find_eligible_providers_for_request(p_request_id uuid, p_limit integer default 20)
...
begin
  ...
  if actor_account is null then raise exception 'active account required' using errcode='42501'; end if;
  ...
  if r.service_entity_id is null or r.location_id is null then return; end if;

  return query
  select e.provider_id,e.slug,e.headline,e.public_description,e.readiness_score,e.trust_score,e.verification_summary
  from public.find_eligible_providers(r.service_entity_id,r.location_id,least(greatest(coalesce(p_limit,20),1),50)) e
  join public.providers p on p.id=e.provider_id
  where p.owner_account_id <> actor_account;
end
$function$;

grant execute on function public.find_eligible_providers_for_request(uuid,integer) to authenticated;
