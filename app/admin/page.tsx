...
export default async function AdminHome() {
  ...
  const alertCount = Number(o.providers?.pending_verifications ?? 0) + Number(o.work?.disputed ?? 0) + Number(o.money?.disputed_obligations ?? 0) + Number(o.operations?.rejected_provider_events ?? 0);

  return <div className="admin-page">
    <header className="admin-page-header"><div><p className="eyebrow">Platform overview</p><h1>See what needs attention.</h1><p>Important work first. Detailed controls stay one click away.</p></div><div className={`admin-health ${alertCount ? 'attention' : ''}`}><strong>{alertCount}</strong><span>{alertCount ? 'items need attention' : 'no critical alerts'}</span></div></header>
    ...
    <section className="admin-stat-grid" aria-label="Platform summary">
      ...
      <article><span>Active providers</span><strong>{o.providers?.active ?? 0}</strong><small>{o.providers?.pending_verifications ?? 0} verification reviews</small></article>
    ...
    <section className="admin-section"><div className="admin-section-heading"><div><h2>Attention queue</h2><p>Exceptions and consequential work, grouped for quick action.</p></div></div>
      <div className="admin-action-grid">
        ...
        <Link href="/admin/operations"><strong>Provider event failures</strong><span>{o.operations?.rejected_provider_events ?? 0} rejected</span></Link>
      </div>
    </section>

    <section className="admin-section"><div className="admin-section-heading"><div><h2>Control areas</h2><p>Access appears only when your capabilities allow it.</p></div></div>
      <div className="admin-module-grid">
        <Link href="/admin/access"><strong>Users & access</strong><span>Owner, admins, roles and permissions</span></Link>
